...
from dataset.jsonl_abstract_trace_dataset import JSONLAbstractTraceDataset
from dataset.jsonl_moler_trace_dataset import JSONLMoLeRTraceDataset
from models.moler_vae import MoLeRVae
from utils.cli_utils import setup_logging, supress_tensorflow_warnings
import os
...
def run_from_args(args: argparse.Namespace) -> Tuple[str, str, str]:
    ...
    if not isinstance(loaded_model_dataset[1], MoLeRVae):
        raise ValueError(
            f"This training script can only work with CGVAE/MoLeR, "
            f"but got {loaded_model_dataset[1]}!"
        )
    model: MoLeRVae = loaded_model_dataset[1]
...
def train(
    ...
    with dataset.get_context_managed_tf_dataset(training_utils.DataFold.TRAIN) as train_data:
        ...
        with dataset.get_context_managed_tf_dataset(
            ...
            # print("**********")
            # print(valid_data.tf_dataset)
            tf.print(valid_data.tf_dataset)
...
def get_argparser() -> argparse.ArgumentParser:
    parser = training_utils.get_train_cli_arg_parser(task='MoLeR', data_path='../zinc_trace_dir', default_model_type="GNN_Edge_MLP")
    parser.add_argument(
        "--profile",
        dest="profile",
