...
from chem.atom_feature_utils import (
    AtomFeatureExtractor,
    AtomTypeFeatureExtractor,
)
from chem.molecule_dataset_utils import BOND_DICT
from chem.rdkit_helpers import compute_canonical_atom_order, get_atom_symbol
from chem.motif_utils import (
    find_motifs_from_vocabulary,
    MotifVocabulary,
)
from utils.moler_decoding_utils import (
    DecoderSamplingMode,
...
from utils.decoder_batching import batch_decoder_states
...
class MoLeRDecoder(tf.keras.layers.Layer):
    ...
    >>> from chem.atom_feature_utils import AtomTypeFeatureExtractor
    ...
    def decode(
        ...
        while num_steps < max_num_steps:
            ...
            for decoder_state, (node_type_picks, node_type_logprobs) in zip(
                ...
                # print('node_type_picks', len(node_type_picks))
                for node_type_pick, node_type_logprob in node_type_picks:
                    # Set up generation trace storing variables, populating if needed.
                    node_type_pick = input_molecule
                    atom_choice_info = None
                    if store_generation_traces:
                        atom_choice_info = MoleculeGenerationAtomChoiceInfo(
                    ...
                    # print('node_type_pick', node_type_pick, node_type_logprob)
                    if node_type_pick is None:
                        ...
                        flag = True
                        # print('added_motif',added_motif, Chem.MolToSmiles(new_decoder_state.molecule))
            ...
            if self.uses_motifs:
                ...
                for decoder_state, attachment_point_picks, attachment_point_logits in zip(
                    ...
                    for (attachment_point_pick, attachment_point_logprob) in attachment_point_picks:
                        ...
                        # print('new_decoder_state', Chem.MolToSmiles(new_decoder_state.molecule))
            ...
            for (decoder_state, (bond_picks, edge_choice_info)) in zip(
                ...
                if len(bond_picks) == 0:
                    ...
                    new_decoder_states.append(
                        MoLeRDecoderState.new_with_focus_marked_as_visited(
                            decoder_state,
                            step_info=edge_choice_info,
                            focus_node_finished_logprob=0,
                        )
                    )
                    # print('continue', Chem.MolToSmiles(new_decoder_states[-1].molecule))
                    continue
                ...
                for (bond_pick, bond_pick_logprob) in bond_picks:
                    ...
                    if bond_pick is None:
                        ...
                        # print('bond_pick_none', Chem.MolToSmiles(new_decoder_states[-1].molecule))
                    else:
                        ...
                        # print('pick_bond', Chem.MolToSmiles(new_decoder_states[-1].molecule))
    ...
    def _pick_edges_for_batch(
        ...
        for state_idx, (decoder_state, decoder_state_num_edge_candidates) in enumerate(
            ...
            if store_generation_traces:
                ...
                edge_choice_info = MoleculeGenerationEdgeChoiceInfo(
                    focus_node_idx=decoder_state.focus_atom,
                    partial_molecule_adjacency_lists=decoder_state.adjacency_lists,
                    candidate_edge_infos=edge_candidate_info,
            ...
            picked_edges.append((this_state_results, edge_choice_info))
